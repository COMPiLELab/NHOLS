...
class trans_model(base_model):
    ...
    def build(self):
        ...
        l_x_hid = SparseLayer(l_x_in, self.y.shape[1], nonlinearity = lasagne.nonlinearities.softmax)

        if self.use_feature:
            l_emd_f = DenseLayer(l_emd_f, self.y.shape[1], nonlinearity = lasagne.nonlinearities.softmax)
            l_y = lasagne.layers.ConcatLayer([l_x_hid, l_emd_f], axis = 1)
            l_y = DenseLayer(l_y, self.y.shape[1], nonlinearity = lasagne.nonlinearities.softmax)
        else:
            l_y = DenseLayer(l_emd_f, self.y.shape[1], nonlinearity = lasagne.nonlinearities.softmax)
        ...
        if self.neg_samp == 0:
            l_gy = DenseLayer(l_emb_in, num_ver, nonlinearity = lasagne.nonlinearities.softmax)
